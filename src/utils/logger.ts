...
export class Logger {
	...
	async logHighlightDebugToFile(step: string, data: any): Promise<void> {
		try {
			const { logMobileEvent } = await import("./zora-mobile-logger");
			logMobileEvent("HighlightDebug", step, typeof data === "object" && data !== null ? data : { data });
		} catch {
			// Fail-safe
		}
	}
	...
	suspendVerboseLogs(): void {
		this.verboseSilenceDepth++;
	}

	resumeVerboseLogs(): void {
		this.verboseSilenceDepth = Math.max(0, this.verboseSilenceDepth - 1);
	}

	private isVerboseLoggingEnabled(): boolean {
		return this.debugMode && this.verboseSilenceDepth === 0;
	}
